...
const examples = [
" Assist with making calls and sending messages",

" Help with general knowledge and trivia questions",

" Translate words and phrases in different languages.",
"Help with mathematical calculations and conversions.",
" Set up routines and automation for smart home devices.",
"Provide daily inspiration and motivational quotes.",
...
const Chat = () => {
  ...
  const handleSend = async () => {
    if (input.trim) {
      ...
      setInput('');
      const response = await fetch('https://plprdj-8000.csb.app/api/chat', {
        method: 'POST',
        headers: {
      ...
      if (!title) {
        const createTitle = await fetch('https://plprdj-8000.csb.app/api/title', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    <div className=' h-screen w-screen flex bg-[#0c0c15]'>
      <div className=' w-[18%] h-screen bg-[#050509] text-white p-4'>
        <div className=' h-[5%]'>
        ...
        <div className=' h-[70%] overflow-scroll shadow-lg hide-scroll-bar mb-4'>
          {
            chatHistory.map((item, index) => (
              <div className=' py-3 text-center rounded mt-5 text-md font-md flex items-center px-8 hover:bg-slate-600 cursor-pointer'>
                <span className=' mr-4'>
        ...
        <div className='overflow-scroll shadow-lg hide-scroll-bar h-[20%] border-t'>
          {
            ["Upgrade Premium", "Code Setting"," None Scroll"].map((item, index) => (
              <div className=' py-3 text-center rounded mt-1 text-md font-md flex items-center px-8 hover:bg-slate-600 cursor-pointer'>
                <span className=' mr-4'>
      ...
      <div className=' w-[80%]'>
          ...
          chat.length > 0 ?
            ...
            (
              <div className=' h-[80%] flex flex-col justify-center items-center text-white'>
                <div className=' text-4xl font-bold mb-8'>Genius A! Assistant</div>
                <div className=' flex flex-wrap justify-around max-w-[900px]'>
                  {
                    examples.map((item, index) => (
                      <div className=' text-md font-md mt-4 p-4 border rounded cursor-pointer min-w-[400px] hover:bg-slate-800' onClick={() => setInput(item)}>{item}</div>
                    ))
                  }
                </div>
              </div>
        ...
        <div className=' h-[20%]'>
          <div className=' flex flex-col items-center justify-center w-full h-full text-white'>
            ...
            <small className=' text-slate-500 mt-2'>AI can generate incorrect information. <span className='text-white'>©Mohit Rajpoot</span></small>
          </div>
        </div>
      </div>
    </div>
